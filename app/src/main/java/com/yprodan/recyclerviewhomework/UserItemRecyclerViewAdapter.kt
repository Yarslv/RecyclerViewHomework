    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UserHolder {
        UserItemRecyclerViewBinding.bind(LayoutInflater.from(parent.context).inflate(R.layout.user_item_recycler_view, parent, false)).apply {
            this.trashImageView.setOnClickListener {
                (users as ArrayList<User>).removeAt(this.hack!!.a)
                notifyItemRemoved(this.hack!!.a)

                Toast.makeText(parent.context, "${this.firstNameTextView.text} == ${this.hack?.a} == ${users.size}", Toast.LENGTH_SHORT).show()
            }
