    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UserHolder2 {
        ContactItemRecyclerViewBinding.bind(LayoutInflater.from(parent.context).inflate(R.layout.contact_item_recycler_view, parent, false)).apply {
            this.trashImageView.setOnClickListener {
                if(trashImageView.drawable.toBitmap().sameAs(ResourcesCompat.getDrawable(parent.context.resources, R.drawable.added_icon, null)?.toBitmap()))
                    this.trashImageView.setImageDrawable(ResourcesCompat.getDrawable(parent.context.resources, R.drawable.add_icon, null))
                else
